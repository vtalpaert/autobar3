<script lang="ts">
    ...
    $: filteredOrders = data.activeOrders
        ? data.activeOrders.filter((order) => order.deviceId === selectedDeviceId)
        : [];
            ...
            >
                {#each data.devices as device}
                    <option value={device.id}
                        >{device.name || `Device ${device.id.slice(0, 8)}`} - Owner: {device.ownerUsername}</option
                    >
                {/each}
            </select>
            ...
            {#if selectedDevice}
                <div class="mt-4">
                    <p class="mb-2 text-gray-300">
                        Selected: <strong
                            >{selectedDevice.name ||
                                `Device ${selectedDevice.id.slice(0, 8)}`}</strong
                        >
        ...
        <div class="mb-8 p-6 bg-gray-800 border border-gray-700 rounded-lg shadow-lg">
            ...
            {#if currentAction}
                <div class="mb-4 p-4 bg-gray-700 rounded">
                    ...
                    {#if currentAction === 'pump'}
                        ...
                        <p>
                            <strong>Remaining to Pump:</strong>
                            {currentDoseWeight - currentWeightProgress}g
                        </p>
            ...
            <div class="mb-4 p-4 bg-gray-700 rounded">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="manualOrderId" class="block text-sm font-medium mb-1"
                            >Order ID:</label
                        >
                        <input
                            id="manualOrderId"
                    ...
                    <div>
                        <label for="manualDoseId" class="block text-sm font-medium mb-1"
                            >Dose ID:</label
                        >
                        <input
                            id="manualDoseId"
            ...
            {#if (currentAction === 'pump' || useManualIds) && selectedDevice}
                <div class="mb-4">
                    <h3 class="font-semibold mb-2">
                        Set Progress Weight (absolute weight in grams):
                    </h3>
                    {#if currentAction === 'pump' && !useManualIds}
                        <p class="text-sm text-gray-300 mb-2">
                            Total dose: {currentDoseWeight}g | Current progress: {currentWeightProgress}g
                        ...
                        {#each [5, 10, 20, 30, 50] as weightG}
                            <form method="POST" action="?/progress" use:enhance class="inline">
                                ...
                                <input
                                    type="hidden"
                                    name="orderId"
                                    value={useManualIds ? manualOrderId : currentOrderId}
                                />
                                <input
                                    type="hidden"
                                    name="doseId"
                                    value={useManualIds ? manualDoseId : currentDoseId}
                                />
                                ...
                                {#if simulateAsDevice}
                                    <input
                                        type="hidden"
                                        name="overrideDeviceId"
                                        value={simulateAsDevice.id}
                                    />
                                {/if}
                                <button
                                    type="submit"
            ...
            {#if (currentAction === 'pump' || useManualIds) && selectedDevice}
                <div class="mt-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        {#each [{ msg: 'General error', code: '1' }, { msg: 'Weight scale error', code: '2' }, { msg: 'No weight change (pump/reservoir issue)', code: '3' }, { msg: 'Negative weight change', code: '4' }, { msg: 'Unable to report progress', code: '5' }] as error}
                        ...
                        >
                            ...
                            <div class="text-sm text-gray-300 space-y-1">
                                <p>
                                    ...
                                    {order.customerArtistName || order.customerUsername}
                                </p>
                                <p>
                                    <strong>Order ID:</strong>
                                    <span class="font-mono text-xs bg-gray-800 px-1 rounded"
                                        >{order.id}</span
                                    >
                                </p>
                                ...
                                {#if order.currentDoseId}
                                    <div class="mt-2 p-2 bg-gray-600 rounded">
                                        <p class="font-medium">
                                            Current Dose #{order.currentDoseNumber}:
                                        </p>
                                        <p>
                                            <strong>Dose ID:</strong>
                                            <span class="font-mono text-xs bg-gray-800 px-1 rounded"
                                                >{order.currentDoseId}</span
                                            >
                                        </p>
                                        <p>
                                            <strong>Ingredient:</strong>
                                            {order.currentIngredientName || 'Unknown'}
                                        </p>
                                        <p><strong>Weight:</strong> {order.currentDoseQuantity}g</p>
                                        <p><strong>Progress:</strong> {order.doseProgress || 0}g</p>
                                        <p>
                                            <strong>Remaining:</strong>
                                            {order.currentDoseQuantity - (order.doseProgress || 0)}g
                                        </p>
                                    </div>
                                {:else}
