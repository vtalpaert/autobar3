<script lang="ts">
    ...
    function getAvailabilityBadge(cocktail: any) {
        switch (cocktail.availability) {
            ...
            case 'partial':
                return {
                    text: `${t.cocktails.missingIngredients} ${cocktail.missingIngredients.length}`,
                    class: 'bg-yellow-600'
                };
            case 'unavailable':
    ...
    function getActionButton(cocktail: any) {
        if (cocktail.availability === 'available') {
            return {
                text: t.cocktails.makeNow,
                class: 'bg-green-600 hover:bg-green-700',
                action: 'order'
            };
        } else if (cocktail.availability === 'no-device') {
            return {
                text: t.cocktails.setupDevice,
                class: 'bg-blue-600 hover:bg-blue-700',
                action: 'setup'
            };
        } else {
            return {
                text: t.cocktails.viewDetails,
                class: 'bg-blue-600 hover:bg-blue-700',
                action: 'view'
            };
        }
    ...
    <div class="container mx-auto px-4 py-16">
        ...
        {#if data.defaultDevice}
            {#if data.deviceCapabilities?.needsCalibration}
                <div class="bg-yellow-600 text-white p-4 rounded-lg mb-8">
                    ...
                    <p>
                        Device "{data.defaultDevice.name || 'Unnamed'}" {t.cocktails
                            .needsCalibrationMessage}
                    </p>
                </div>
            {:else}
                <div class="bg-green-600 text-white p-4 rounded-lg mb-8">
                    ...
                    <p>
                        Device "{data.defaultDevice.name || 'Unnamed'}" - {data.deviceCapabilities
                            ?.availableIngredients?.length || 0}
                        {t.cocktails.ingredientsAvailable}
                    </p>
                </div>
            {/if}
        {:else}
            <div class="bg-red-600 text-white p-4 rounded-lg mb-8">
                ...
                <p>
                    {t.cocktails.setUpDefaultDevice}
                    <a href="/devices" class="underline">{t.cocktails.configureDevice}</a>
                </p>
            </div>
        {/if}
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {#each filteredCocktails as cocktail}
                ...
                <div
                    class="bg-gray-800 rounded-lg shadow-lg overflow-hidden border-2 {cocktail.availability ===
                    'available'
                        ? 'border-green-500'
                        : cocktail.availability === 'partial'
                          ? 'border-yellow-500'
                          : cocktail.availability === 'unavailable'
                            ? 'border-red-500'
                            : 'border-gray-600'}"
                >
                    <!-- Image with Badge -->
                    <div class="relative">
                        ...
                        <div class="absolute top-2 right-2">
                            <span
                                class="px-2 py-1 text-xs font-bold text-white rounded {badge.class}"
                            >
                                {badge.text}
                            </span>
                        </div>
                    ...
                    <div class="p-6">
                            ...
                            <div class="mb-4 text-sm">
                                {#if cocktail.availableIngredients.length > 0}
                                    <p class="text-green-400">
                                        ✓ {t.cocktails.availableLabel}: {cocktail.availableIngredients.join(
                                            ', '
                                        )}
                                    </p>
                                {/if}
                                {#if cocktail.missingIngredients.length > 0}
                                    <p class="text-red-400">
                                        ⚠ {t.cocktails.missingLabel}: {cocktail.missingIngredients.join(
                                            ', '
                                        )}
                                    </p>
                                {/if}
                                {#if cocktail.manualIngredients.length > 0}
                                    <p class="text-blue-400">
                                        ℹ {t.cocktails.manualLabel}: {cocktail.manualIngredients.join(
                                            ', '
                                        )}
                                    </p>
                                {/if}
                        ...
                        <div class="flex gap-2">
                            ...
                            {#if actionBtn.action === 'order' && cocktail.availability === 'available'}
                                <form
                                    method="POST"
                                    action="/cocktails/{cocktail.id}?/createOrder"
                                    use:enhance
                                    class="flex-1"
                                >
                                    <button
