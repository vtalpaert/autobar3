...
export const actions: Actions = {
    ...
    savePins: async ({ request, locals, params }) => {
        ...
        try {
            ...
            await db.update(table.device).set(updateData).where(eq(table.device.id, deviceId));

            return {
                success: true,
                message: 'GPIO pins saved. Device will reinitialize hardware.'
            };
        } catch (error) {
    ...
    saveCalibration: async ({ request, locals, params }) => {
        ...
        if (isNaN(offset) || isNaN(scale) || scale === 0) {
            return fail(400, {
                success: false,
                message: 'Valid offset and scale values are required'
            });
        }
        ...
        try {
            ...
            return {
                success: true,
                message: 'Calibration saved successfully. Device is now ready to use.'
            };
        } catch (error) {
