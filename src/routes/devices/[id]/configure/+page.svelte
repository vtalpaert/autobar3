<script lang="ts">
    ...
    function addLocalPump() {
        ...
        const existingPump = pendingPumps.find(p => 
    ...
    <div class="container mx-auto px-4 py-16">
        <div class="max-w-4xl mx-auto bg-gray-800 rounded-lg shadow-lg p-8">
            ...
            <div class="mb-8 p-6 bg-gray-700 rounded-lg">
                <div class="flex flex-wrap items-center gap-2 mb-4">
                    ...
                    {#if data.device.lastPingAt && isDeviceOnline(data.device.lastPingAt)}
                        <span class="bg-green-500 text-white text-xs px-2 py-1 rounded flex items-center gap-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        <p class="text-gray-400">{t.devices.configure.deviceId}: {data.device.id.slice(0, 8)}</p>
                        <p class="text-gray-300">{t.devices.configure.firmware}: {data.device.firmwareVersion}</p>
                    </div>
                    <div>
                        {#if data.device.lastUsedAt}
                            <p class="text-gray-400">{t.devices.configure.lastUsed}: {getRelativeTime(data.device.lastUsedAt)}</p>
                        {:else}
                            <p class="text-gray-500">{t.devices.configure.neverUsed}</p>
                        {/if}
                        {#if data.device.lastPingAt}
                            <p class="text-gray-400">{t.devices.configure.lastSeen}: {getRelativeTime(data.device.lastPingAt)}</p>
                        {:else}
            ...
            <form method="POST" use:enhance>
                            ...
                            {#each existingPumps as pump, index}
                                <div class="p-4 bg-gray-700 rounded-lg">
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                        <div>
                                            <label for="existing-ingredient-{index}" class="block text-sm font-medium mb-2">
                                            ...
                                            >
                                                <option value="">{t.devices.configure.noIngredient}</option>
                                        ...
                                        <div>
                                            <label for="existing-gpio-{index}" class="block text-sm font-medium mb-2">
                                    ...
                                    <input type="hidden" name={`existingPumps[${index}].id`} value={pump.id} />
                                    <input type="hidden" name={`existingPumps[${index}].ingredientId`} value={pump.ingredientId} />
                                    <input type="hidden" name={`existingPumps[${index}].gpio`} value={pump.gpio === '' ? '' : pump.gpio} />
                                    <input type="hidden" name={`existingPumps[${index}].isEmpty`} value={pump.isEmpty ? 'on' : ''} />
                                </div>
                            {/each}
                        </div>
                    </div>
                            ...
                            {#each pendingPumps as pump, index}
                                <div class="p-4 bg-gray-700 rounded-lg flex items-center justify-between">
                                    <div>
                                        ...
                                        {/if}
                                        {#if pump.isEmpty}
                                            <span class="ml-2 text-yellow-400">({t.devices.configure.empty})</span>
                                        {/if}
                                    </div>
                                    <button 
                                        type="button"
                    ...
                    {#if showAddPump}
                        <div class="mt-4 p-4 bg-gray-700 rounded-lg mb-6">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                                <div>
                                    <label for="new-ingredient" class="block text-sm font-medium mb-2">
                                        {t.devices.configure.ingredient}
                                    </label>
                                    <select
                                        id="new-ingredient"
                                        ...
                                        class="w-full bg-gray-800 text-white rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    >
                                        <option value="">{t.devices.configure.chooseIngredient}</option>
                                        {#each data.ingredients as ingredient}
                                            <option value={ingredient.id}>{ingredient.name}</option>
                                        {/each}
                                    </select>
                ...
                {#each pendingPumps as pump, index}
                    <input type="hidden" name={`newPumps[${index}].ingredientId`} value={pump.ingredientId} />
                    <input type="hidden" name={`newPumps[${index}].gpio`} value={pump.gpio || ''} />
                    <input type="hidden" name={`newPumps[${index}].isEmpty`} value={pump.isEmpty ? 'on' : ''} />
                {/each}
