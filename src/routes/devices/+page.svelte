<script lang="ts">
    ...
    function getRelativeTime(dateString: string): string {
        ...
        if (diffMins < 60)
            return $currentLanguage === 'fr' ? `Il y a ${diffMins}min` : `${diffMins}min ago`;
        if (diffHours < 24)
            return $currentLanguage === 'fr' ? `Il y a ${diffHours}h` : `${diffHours}h ago`;
        if (diffDays < 7)
            return $currentLanguage === 'fr' ? `Il y a ${diffDays}j` : `${diffDays}d ago`;
                    ...
                    {#if editingDeviceId === device.id}
                        <form method="POST" action="?/renameDevice" use:enhance={handleEnhance}>
                            ...
                            <div class="flex flex-col space-y-2 mb-4">
                                <label for="deviceName" class="text-sm text-gray-400"
                                    >{t.devices.deviceName}</label
                                >
                                <input
                                    type="text"
                                    id="deviceName"
                        ...
                        >
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-wrap items-center gap-2 mb-2">
                                    ...
                                    {#if device.isDefault}
                                        <span
                                            class="bg-green-600 text-white text-xs px-2 py-1 rounded whitespace-nowrap"
                                        >
                                            {t.devices.default}
                                        </span>
                                    {/if}
                                    {#if device.lastPingAt && isDeviceOnline(device.lastPingAt)}
                                        <span
                                            class="bg-green-500 text-white text-xs px-2 py-1 rounded whitespace-nowrap flex items-center gap-1"
                                        >
                                            <div
                                    ...
                                    {:else if device.lastPingAt}
                                        <span
                                            class="bg-gray-500 text-white text-xs px-2 py-1 rounded whitespace-nowrap"
                                        >
                                            {t.devices.offline}
                                        </span>
                                    {:else}
                                        <span
                                            class="bg-red-500 text-white text-xs px-2 py-1 rounded whitespace-nowrap"
                                        >
                                    ...
                                    {#if device.needCalibration}
                                        <span
                                            class="bg-yellow-600 text-white text-xs px-2 py-1 rounded whitespace-nowrap"
                                        >
                                ...
                                <div class="space-y-1 text-sm">
                                    <p class="text-gray-400">
                                        {t.devices.deviceId}: {device.id.slice(0, 8)}
                                    </p>
                                    <p class="text-gray-300">
                                        {t.devices.firmware}: {device.firmwareVersion}
                                    </p>
                                    <p class="text-gray-400">
                                    ...
                                    {#if device.lastUsedAt}
                                        <p class="text-gray-400">
                                            {t.devices.lastUsed}: {getRelativeTime(
                                                device.lastUsedAt
                                            )}
                                        </p>
                                    {:else}
                                    ...
                                    {#if device.lastPingAt}
                                        <p class="text-gray-400">
                                            {t.devices.lastSeen}: {getRelativeTime(
                                                device.lastPingAt
                                            )}
                                        </p>
                                    {:else}
