...
import { getCocktailImagePath, getCocktailImageUri, ensureFileDirectoryExists } from './index';
...
export async function saveCocktailImage(
    ...
    try {
        ...
        throw new Error(
            `Failed to save cocktail image: ${error instanceof Error ? error.message : 'Unknown error'}`
        );
    }
}
