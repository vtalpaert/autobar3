<script lang="ts">
    ...
    export let animatingOrders: Map<
        string,
...
<div
    id="order-{order.id}"
    class="bg-gray-800 rounded-lg shadow-lg overflow-hidden mb-6 transition-all duration-300 {animatingOrders.has(
        order.id
    )
        ? `order-${animatingOrders.get(order.id)?.type}`
        : ''} relative {isRecentlyCompleted ? 'opacity-90' : ''}"
>
    ...
    <div class="flex flex-col md:flex-row">
        ...
        <div class="flex-1 p-6">
                ...
                <div class="bg-gray-700 rounded-lg p-4 mb-4">
                    <div class="flex items-center justify-between mb-2">
                        <span class="text-sm font-medium text-white">
                            {t.myBar.orders.nowPouring}
                            {order.currentDose.ingredient.name}
                        </span>
                        <span class="text-xs text-gray-400">
                            {t.myBar.orders.stepOf
                                .replace('{current}', order.currentDose.number)
                                .replace('{total}', cocktail?.doses?.length || '?')}
                        </span>
                    </div>
            ...
            {#if cocktail?.doses && cocktail.doses.length > 0}
                <div class="mb-4">
                    <h4 class="text-sm font-medium text-gray-300 mb-3">
                        {t.myBar.orders.recipeTimeline}
                    </h4>
                    <div class="space-y-2">
                        {#each cocktail.doses as dose}
                            {@const isCompleted =
                                order.currentDose && dose.number < order.currentDose.number}
                            {@const isCurrent =
                                order.currentDose && dose.number === order.currentDose.number}
                            {@const isAllCompleted =
                                isRecentlyCompleted && order.status === 'completed'}
                            <div class="flex items-center space-x-3">
                                ...
                                <div class="flex-1">
                                    <span
                                        class={`text-sm ${isCurrent ? 'text-white font-medium' : 'text-gray-400'}`}
                                    >
            ...
            {#if cocktail?.instructions}
                <div class="mb-4">
                    <h4 class="text-sm font-medium text-gray-300 mb-2">
                        {t.myBar.orders.preparationInstructions}
                    </h4>
                    <div
                        class="p-3 bg-gray-700 rounded text-sm text-gray-300 leading-relaxed whitespace-pre-wrap"
                    >
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-gray-400">
                ...
                <div>
                    <p>
                        {t.myBar.orders.device}: {order.device
            ...
            {#if order.errorMessage}
                <div
                    class="mt-4 bg-red-900/20 border border-red-800/30 text-red-400 px-4 py-3 rounded"
                >
...
<style>
    ...
    :global(.order-cancelled) {
        ...
        animation:
            stripe-slide 2s linear infinite,
