...
{
    if (get_stored_hx711_config(&weight_scale.hx711.dout, &weight_scale.hx711.pd_sck, &weight_scale.offset, &weight_scale.scale))
    {
...
bool weight_interface_need_calibration()
{
    ...
    if (!_measure(&measure))
    {
        ...
        return true; // Assume calibration needed if we can't measure
    }
    ...
    if (server_dt_pin != weight_scale.hx711.dout || 
        server_sck_pin != weight_scale.hx711.pd_sck ||
        ...
        server_scale != weight_scale.scale)
    {
        ...
        if (server_dt_pin != weight_scale.hx711.dout || server_sck_pin != weight_scale.hx711.pd_sck)
        {
            weight_scale.hx711.dout = server_dt_pin;
            weight_scale.hx711.pd_sck = server_sck_pin;
